...
class Game extends Phaser.Scene{
    ...
    create(){
        //some decoration
        this.add.line(400,250,0,0,0,500,0xffffff,1).setStrokeStyle(5,0xffffff,1);
    ...
    update(){
        ...
        if (this.cursors.up.isDown) {
            this.leftPaddle.body.setVelocityY(-200); // Move ups
        }
        ...
        //lets have the right padding move on its ownw
        const far = this.rightPaddle.x - this.ball.x
        const diff = this.ball.y - this.rightPaddle.y;
        if (Math.abs(diff) > 30 & far < 200){
          if(diff < 0){//ball is above the paddle
                this.rightPaddle.body.setVelocityY(-200)
            }else if(diff > 0){//ball is under paddle
                this.rightPaddle.body.setVelocityY(200)
            } 
            }else{
